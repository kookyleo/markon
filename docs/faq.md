...
没有按账号的角色系统。Markon 只按**访问来源**分权：**本机（loopback）等同管理员、拥有全部能力且免码**；**远程访客是协作者，能做什么由该工作区的功能开关决定**（如 `edit`、`chat`、共享便条）。远程不能做管理 / 结构性操作。适合小团队 wiki 或读书会场景，不适合需要细粒度按人授权的正式文档系统。详见[访问权限](/features/access)。
