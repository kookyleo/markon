...
没有按账号逐人授权的角色系统。浏览器默认是协作者，能力由工作区功能开关决定（如 `edit`、`chat`、共享批注）；管理 / 结构性操作需要通过 `markon admin open` 或 `markon admin code` 创建显式管理员会话。loopback 与反向代理不会自动提升权限。适合小团队 wiki 或读书会场景，不适合需要细粒度按人授权的正式文档系统。详见[访问权限](/features/access)。

### 想给工作区加把锁、分享时让对方先验证怎么办？

用[协作者访问码门禁](/features/access)。你可以在 GUI 里设全局口令，也可以为单个工作区单独设口令；之后所有未持有管理员会话的浏览器都要在门禁页输入正确口令，本机 URL 也不例外。注意这只是应用层的简单防护，真要对外暴露请配合 TLS / 反向代理。

### 能和 Obsidian / VS Code 共享批注数据吗？

不能。Markon 的批注格式是自己的（基于 XPath 定位），无法与其他工具互通。
