...
没有按账号的角色系统。Markon 只按**访问来源**分权：**本机（loopback）等同管理员、拥有全部能力且免码**；**远程访客是协作者，能做什么由该工作区的功能开关决定**（如 `edit`、`chat`、共享批注）。远程不能做管理 / 结构性操作。适合小团队 wiki 或读书会场景，不适合需要细粒度按人授权的正式文档系统。详见[访问权限](/features/access)。

### 想给工作区加把锁、分享时让对方先验证怎么办？

用[协作者访问码门禁](/features/access)。你可以在 GUI 里设全局口令，也可以为单个工作区单独设口令；之后**远程访客**要在浏览器门禁页输入正确口令才能进入（本机始终免码放行）。注意这只是应用层的简单防护，真要对外暴露请配合 TLS / 反向代理。
