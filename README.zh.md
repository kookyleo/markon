...
通过 Cargo 安装已发布的 CLI。请一并安装 `markond` —— 它是 `markon` 命令拉起的后台服务，
必须与其一同位于 `PATH` 上(缺少它时,`markon` 会退化为在前台运行):
...
cd markon
cargo install --path crates/markond   # 后台服务
cargo install --path crates/cli       # `markon` 命令
```
...
| `markon cleanup [--yes]` | 查看统计并清理不属于任何活动工作区的数据 |
...
| 共享批注 | 批注与 Viewed 始终持久化到 SQLite；开启后再通过 WebSocket 向协作者实时同步 |
...
| 个人/共享批注、Viewed 状态、AI 对话会话 | `~/.markon/annotation.sqlite` |
| 旧版本批注迁移与无权限降级 | 浏览器 LocalStorage（成功迁移后自动清除当前来源副本） |
...
| 路径 | 职责 |
| --- | --- |
| `crates/core` | HTTP 服务、渲染、搜索、持久化、Git、Chat、控制套接字协议与浏览器资源 |
| `crates/markond` | 后台服务进程 —— 唯一持有 core，同时提供 Web(TCP)与控制(本地套接字)两个平面 |
| `crates/cli` | `markon` CLI —— 特权本地客户端,通过控制套接字拉起 / 挂接 `markond` |
| `crates/gui` | Tauri 2 桌面壳与设置界面 —— 基于同一控制套接字的纯前端 |
