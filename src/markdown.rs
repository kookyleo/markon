...
impl MarkdownRenderer {
                ...
                Event::End(TagEnd::CodeBlock) => {
                    if in_code_block {
                        ...
                        if code_lang.to_lowercase() == "mermaid" {
                            ...
                            // 普通代码块，进行语法高亮
                            let syntax = ss.find_syntax_by_extension(&code_lang)
                                ...
                                match highlighter.highlight_line(line, &ss) {
                                    Ok(ranges) => {
                                        match styled_line_to_highlighted_html(&ranges[..], IncludeBackground::No) {
                                            ...
                                            Err(_) => highlighted_html.push_str(&html_escape::encode_text(line)),
                                        }
                                    }
                                    Err(_) => highlighted_html.push_str(&html_escape::encode_text(line)),
                                }
    ...
    fn process_github_alerts(&self, html: &str) -> String {
        ALERT_REGEX.replace_all(html, |caps: &regex::Captures| {
    ...
    fn generate_alert_html(&self, alert_type: &str, content: &str) -> String {
        ...
        format!(
            r#"<div class="markdown-alert markdown-alert-{}">
...
{}
    ...
    fn add_heading_ids_and_extract_toc(&self, html: &str) -> (String, Vec<TocItem>) {
        ...
        let result = HEADING_REGEX.replace_all(html, |caps: &regex::Captures| {
